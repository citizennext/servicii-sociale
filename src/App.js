...
class App extends Component {
	constructor(props) {
		...
		this.state = {
			...
			type: [
				{
					name: 'Centru rezidenţial cu cazare pe perioadă nedeterminată',
					code: '1',
				},
				{
					name: 'Centru rezidenţial cu cazare pe perioadă determinată',
					code: '2',
				},
	...
	componentDidMount() {
		axios
			...
			.then(data => {
				...
				const districts = districtsMap
					.filter((v, i) => districtsMap.indexOf(v) === i)
					.sort()
				const servicesMap = data.data.ss.map(service => service.cat)
				const categories = servicesMap.filter(
					(v, i) => servicesMap.indexOf(v) === i
				)
	...
	handleDisclaimer = () => {
		this.setState({ disclaimerOpen: !this.state.disclaimerOpen })
	}
					...
					{this.state.isLoading === true
						...
						: <div
							...
							>
								...
								>
									Harta serviciilor sociale licențiate este în versiune Beta. În
									această fază, softul este testat în practică de către
									utilizatori, iar apoi vor fi făcute modificări pentru a
									înlătura eventualele erori. Datele prezentate pe platformă
									sunt la nivelul lunii septembrie a anului 2016 și sunt în curs
									de actualizare. Utilizatorii pot contribui la acuratețea
									datelor prin trimiterea de observații și propuneri de
									îmbunătățire, la adresa de email{' '}
									<a href="mailto:acreditare@mmuncii.ro">
										acreditare@mmuncii.ro
									</a>
									.
									<IconButton
										onTouchTap={this.handleDisclaimer}
