...
const MarkerClustererGoogleMap = withGoogleMap(props => {
  const filterData = (markers, selectedFilters) => {
    const activeFilterKeys = R.keys(
      R.filter(R.identity, R.dissoc('categorie', selectedFilters))
    );

    const filterInType = d =>
      !selectedFilters.categorie ||
      d.categorie.indexOf(selectedFilters.categorie) >= 0;
    return R.compose(
  ...
  const setLat = filterData(props.markers, props.selectedFilters).map(
    marker => marker.lat
  );
  const setLng = filterData(props.markers, props.selectedFilters).map(
    marker => marker.lng
  );
      ...
      <GoogleMap
        ...
        {R.isEmpty(props.selectedFilters)
          ? <MarkerClusterer averageCenter enableRetinaIcons gridSize={60}>
              {filterData(props.markers, props.selectedFilters).map(marker => (
                <Marker
                  position={{
