...
belle.style.select.style = R.merge(belle.style.select.style, {
	color: '#000',
	borderBottom: '2px solid #5dd3ec',
	margin: '0 20px',
	width: 214,
})
belle.style.select.hoverStyle = R.merge(belle.style.select.hoverStyle, {
	borderBottom: '2px solid #5dd3ec',
})
belle.style.select.wrapperStyle = R.merge(belle.style.select.wrapperStyle, {
	padding: '10px 0 5px 0',
})
belle.style.select.focusStyle = R.merge(belle.style.select.focusStyle, {
	borderBottom: '2px solid #5dd3ec',
})
belle.style.select.menuStyle = R.merge(belle.style.select.menuStyle, {
	marginLeft: '20px',
	width: 220,
	zIndex: 1300,
})
belle.style.select.caretToOpenStyle = R.omit(
	['content'],
	belle.style.select.caretToOpenStyle
)
belle.style.select.caretToCloseStyle = R.omit(
	['content'],
...
const Sidebar = props => {
	let filteredServices = props.selectedFilters.category
		? props.servicii.filter(
				service => service.cat === props.selectedFilters.category
			)
	...
	return (
		...
		>
				...
				<div className="filter">
					<div className="filter-child">
						<Select
							className="combo"
							value={
								props.selectedFilters.jud !== undefined
									? props.selectedFilters.jud
									: ''
							}
							onUpdate={event => {
								props.changeDistrict(event.value)
							}}
						...
						>
							...
							{props.districts.map((district, index) =>
								<Option value={district} identifier={index} key={index}>
									{district}
								</Option>
							)}
						</Select>
						<Select
							className="combo"
							value={
								props.selectedFilters.category !== undefined
						...
						>
							...
							{props.categories.map((category, index) =>
								<Option value={category} identifier={index} key={index}>
									{category}
								</Option>
							)}
						</Select>
						<Select
							className="combo"
							value={
								props.selectedFilters.cod !== undefined
									? props.selectedFilters.cod
									: ''
							}
						...
						>
							...
							{filteredServices.map((service, index) =>
								<Option value={service.cod} identifier={index} key={index}>
									{service.desc}
								</Option>
							)}
						</Select>
						<Select
							className="combo"
							value={
								props.selectedFilters.type !== undefined
									? props.selectedFilters.type
									: ''
							}
							onUpdate={event => {
								props.changeType(event.value)
							}}
							menuStyle={{ maxHeight: 250, overflow: 'scroll' }}
						>
							<Option defaultValue value="">
								Forma organizare serviciu
							</Option>
						...
						<Select
							className="combo"
							value={
								props.selectedFilters.categorie !== undefined
									? props.selectedFilters.categorie
									: ''
							}
							onUpdate={event => {
								props.changeBeneficiary(event.value)
							}}
						...
						>
							...
							{props.beneficiaries.map((beneficiary, index) =>
								<Option value={beneficiary.code} key={index}>
									{beneficiary.name}
								</Option>
							)}
						</Select>
			...
			<IconButton
				onTouchTap={props.handleDisclaimer}
				className="meta"
				touch={true}
